...
def findSuitableParcels():
    ui.statusbar.showMessage('Extracting data... please wait!')
    try:
        ...
        parcelsField = ui.parcelsFieldCB.currentText()
        parcelsValue = ui.parcelsValueCB.currentText()
        ####
        poverty = ui.povertyLineEdit.text()
        povertyField = ui.povertyCB.currentText()
        ...
        pop_densityField = ui.pop_densityCB.currentText()
        ...
        aoiLayer.updateExtents()

        aoiLayer_2232 = processing.run("native:reprojectlayer",{"INPUT": aoiLayer,"TARGET_CRS": QgsCoordinateReferenceSystem("EPSG:2232"),"OUTPUT": "memory:"})["OUTPUT"]
        ...
        for layer in layers:
            if layer is None:
                continue
            layerClip = processing.run("qgis:clip", {"INPUT": layer, "OVERLAY": aoiLayer_2232, "OUTPUT": "memory:"})["OUTPUT"]
        ...
        parcelQuery = f'{parcelsField} = \'{parcelsValue}\''
        filteredParcels = filterByQuery(clipped_layers, "Parcels", parcelQuery)
        ...
        povertyQuery = f'{povertyField} >= {povertyValue}'
        ...
        popDensityQuery = f'{pop_densityField} >= {pop_densityValue}'
        ...
        for parcel in updatedParcels.getFeatures():
            ...
            if transitIndex != -1:
                suitability += parcel.attribute("Transit_Score")
        ...
        # if the output vector file does not have any attributes, display an error message
        if updatedParcels.featureCount() == 0:
            QMessageBox.information(mainWindow, 'Operation completed with no output parcels',
                                    'Output file does not have any attributes. Please adjust input values.',
                                    QMessageBox.StandardButton.Ok)

    # if there is an error, display the error message
    except Exception as e:
        print(str(e.__class__) + ': ' + str(e))
        QMessageBox.information(mainWindow, 'Operation failed',
                                'Extracting suitable parcels failed. ' + str(e.__class__) + ': ' + str(e),
                                QMessageBox.StandardButton.Ok)
...
def getLayerFields(filePath):
    layer = QgsVectorLayer(filePath, "input_layer", "ogr") # create the vector file from the file path

    fieldList = []
    fieldNames = layer.fields() # look up the fields in the vector file
    for field in fieldNames:
        fieldList.append(field.name()) # add fields to list
    return fieldList

def GetParcelsValueCB(filePath):
    layer = QgsVectorLayer(filePath, "input_layer", "ogr") # create the vector file from the file path
    currentField = ui.parcelsFieldCB.currentText() # find the field value from the field combobox
    fieldIndex = layer.fields().lookupField(currentField) # find the index of the field

    values = layer.uniqueValues(fieldIndex) # get the unique values of the field
    valueList = []
    for value in values:
        valueList.append(str(value)) # add the values to the value list
    return valueList

def updatePovertyFieldCB():
    ui.povertyCB.clear()
    fileName = ui.povertyLineEdit.text()
    ui.povertyCB.addItems(getLayerFields(fileName))

def updatePopDensityFieldCB():
    ui.pop_densityCB.clear()
    fileName = ui.pop_densityLineEdit.text()
    ui.pop_densityCB.addItems(getLayerFields(fileName))

def updateParcelsFieldCB():
    ui.parcelsFieldCB.clear()
    fileName = ui.parcelsLineEdit.text()
    ui.parcelsFieldCB.addItems(getLayerFields(fileName))

def updateParcelsValueCB():
    ui.parcelsValueCB.clear()
    fileName = ui.parcelsLineEdit.text()
    values = GetParcelsValueCB(fileName)
    ui.parcelsValueCB.addItems(sorted(values))
...
if __name__ == '__main__':
    ...
    ui.parcelsFieldCB.currentIndexChanged.connect(updateParcelsValueCB)
