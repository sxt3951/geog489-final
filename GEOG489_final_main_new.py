...
def getLayerFields(filePath):
    layer = QgsVectorLayer(filePath, "input_layer", "ogr")
...
def GetParcelsValueCB(filePath):
    layer = QgsVectorLayer(filePath, "input_layer", "ogr")
    currentField = ui.parcelsFieldCB.currentText()
    fieldIndex = layer.fields().lookupField(currentField)

    values = layer.uniqueValues(fieldIndex)
    valueList = []
    for value in values:
